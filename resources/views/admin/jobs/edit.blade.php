...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-md-12">
                <h3 class="page-header">Edit Job #{{ $job->job_no }}</h3>
            </div>
        </div>
        ...
        <form action="{{ url("admin/jobs/{$job->job_no}/edit") }}" method="POST" class="tw-mb-5">
            @csrf
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group {{ $errors->has('img_link') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="img_link" name="img_link" class="form-control input-sm" value="{{ old('img_link', $job->img_link ?? '') }}" placeholder="Enter image link">
                        ...
                        <div class="form-group {{ $errors->has('lang_id') ? 'has-error' : '' }}">
                            ...
                            <select name="lang_id" id="lang_id" class="form-control input-sm">
                                @foreach ($language_list as $id => $name)
                                    <option value="{{ $id }}" {{ (int) old('lang_id', $job->lang_id) === $id ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                ...
                <div class="col-md-3">
                    <div class="form-group {{ $errors->has('delete_at') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="delete_at" name="delete_at" class="form-control input-sm number" value="{{ old('delete_at', $job->delete_in_days ?? 30) }}" placeholder="Days to delete">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="title" name="title" class="form-control input-sm" value="{{ old('title', $job->title) }}" placeholder="Enter title">
                ...
                <div class="col-md-6">
                    <div class="form-group {{ $errors->has('company_name') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="company_name" name="company_name" class="form-control input-sm" value="{{ old('company_name', $job->company_name) }}" placeholder="Enter company name">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="description" name="description" class="form-control input-sm" value="{{ old('description', $job->description) }}" placeholder="Enter description">
                    ...
                    <div class="form-group {{ $errors->has('job_category_id') ? 'has-error' : '' }}">
                        ...
                        <select name="job_category_id" id="job_category_id" class="form-control input-sm">
                            @foreach ($job_cat_list as $id => $name)
                                <option value="{{ $id }}" {{ (int) old('job_category_id', $job->job_category_id) === $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group {{ $errors->has('prefecture_id') ? 'has-error' : '' }}">
                        ...
                        <select name="prefecture_id" id="prefecture_id" class="form-control input-sm select2">
                            @foreach ($prefecture_list as $id => $name)
                                <option value="{{ $id }}" {{ (int) old('prefecture_id', $job->prefecture_id) === $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group {{ $errors->has('area_id') ? 'has-error' : '' }}">
                        ...
                        <select name="area_id" id="area_id" class="form-control input-sm select2">
                            @foreach ($area_list as $id => $name)
                                <option value="{{ $id }}" {{ (int) old('area_id', $job->area_id) === $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group {{ $errors->has('station') ? 'has-error' : '' }}">
                        ...
                        <textarea id="station" name="station" class="form-control input-sm" placeholder="Enter station" style="resize: none;">{{ old('station', $job->station) }}</textarea>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="address" name="address" class="form-control input-sm" value="{{ old('address', $job->address) }}" placeholder="Enter address">
                    ...
                    <div class="form-group {{ $errors->has('japanese_level') ? 'has-error' : '' }}">
                        ...
                        <select name="japanese_level" id="japanese_level" class="form-control input-sm">
                            @foreach ($jap_level_list as $id => $name)
                                <option value="{{ $id }}" {{ (int) old('japanese_level', $job->japanese_level) === $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group {{ $errors->has('working_hours') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="working_hours" name="working_hours" class="form-control input-sm" value="{{ old('working_hours', $job->working_hours) }}" placeholder="Enter working hours">
                ...
                <div class="col-md-3">
                    <div class="form-group {{ $errors->has('working_days') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="working_days" name="working_days" class="form-control input-sm" value="{{ old('working_days', $job->working_days) }}" placeholder="Enter working days">
                ...
                <div class="col-md-3">
                    <div class="form-group {{ $errors->has('wage') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="wage" name="wage" class="form-control input-sm" value="{{ old('wage', $job->wage) }}" placeholder="Enter wage">
                        ...
                        @foreach ($wage_type_list as $id => $wageType)
                            @if ($id != 0)
                                <label class="radio-inline">
                                    <input type="radio" name="wage_type_id" value="{{ $id }}" {{ (int) old('wage_type_id', $job->wage_type_id) === $id ? 'checked' : '' }}>
                                    {{ $wageType }}
                                </label>
                            @endif
                        @endforeach
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="wage_detail">Wage Detail :</label>
                        <input type="text" id="wage_detail" name="wage_detail" class="form-control input-sm" value="{{ old('wage_detail', $job->wage_detail ?? '') }}" placeholder="Enter wage detail">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                    ...
                    <div class="form-group {{ $errors->has('trans_exp_id') ? 'has-error' : '' }}">
                        ...
                        <select name="trans_exp_id" id="trans_exp_id" class="form-control input-sm">
                            @foreach ($trans_exp_list as $id => $name)
                                <option value="{{ $id }}" {{ (int) old('trans_exp_id', $job->trans_exp_id) === $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-9">
                    <div class="form-group">
                        <label for="transportation_detail">Transportation Detail :</label>
                        <input type="text" id="transportation_detail" name="transportation_detail" class="form-control input-sm" value="{{ old('transportation_detail', $job->transportation_detail ?? '') }}" placeholder="Enter transportation detail">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group {{ $errors->has('requirement') ? 'has-error' : '' }}">
                        ...
                        <input type="text" id="requirement" name="requirement" class="form-control input-sm" value="{{ old('requirement', $job->requirement) }}" placeholder="Enter requirement">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" id="benefits" name="benefits" class="form-control input-sm" value="{{ old('benefits', $job->benefits ?? '') }}" placeholder="Enter benefits">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-5">
                    <div class="form-group {{ $errors->has('apply_link') ? 'has-error' : '' }}">
                        ...
                        <textarea id="apply_link" name="apply_link" class="form-control input-sm" placeholder="Enter apply link" rows="3" style="resize: none;">{{ old('apply_link', $job->apply_link) }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <select name="user_id" id="user_id" class="form-control input-sm">
                            @foreach ($advertiser_list as $id => $name)
                                <option value="{{ $id }}" {{ (int) old('user_id', $job->user_id) === $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="text-right">
                        <button type="submit" class="btn tw-btn-sky">Update</button>
                        <button type="submit" name="update_and_publish" value="1" class="btn tw-btn-emerald">Update &amp; Publish</button>
                    </div>
                </div>
            </div>

            <input type="hidden" id="job_id" name="job_id" value="{{ $job->id }}">
            <input type="hidden" id="job_no" name="job_no" value="{{ $job->job_no }}">
            <input type="hidden" id="hid_area_id" name="hid_area_id" value="{{ old('area_id', $job->area_id ?? 0) }}">
